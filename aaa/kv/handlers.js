...
export async function getDataset(request, env) {
    ...
    try {
        proxySettings = await env.bn1.get("proxySettings", {type: 'json'});
        warpConfigs = await env.bn1.get('warpConfigs', {type: 'json'});
    } catch (error) {
        console.log(error);
...
export async function updateDataset (request, env) {
    ...
    if (!isReset) {
        try {
            currentSettings = await env.bn1.get("proxySettings", {type: 'json'});
        } catch (error) {
            console.log(error);
    ...
    try {    
        await env.bn1.put("proxySettings", JSON.stringify(proxySettings));
